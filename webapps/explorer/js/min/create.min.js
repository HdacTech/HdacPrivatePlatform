...
p.yPos=c(window).scrollTop();p.result=JSON.stringify(o);return p;};this.init=function(){k();n=false;};this.resume=function(){k();};this.handleClick=function(w,u,p){var s=function(){if(c("[name=user_id]").val().trim()==""){return false;
}if(c("[name=user_name]").val().trim()==""){return false;}if(c("[name=password]").val().trim()==""){return false;}};var v=function(){if(e){return;}if(s()==false){return;
}var y=c("[name=form]").serialize();var x={type:"post",dataType:"json",url:l.url,cache:false,data:y,};c.ajax(x).done(function(A,z,B){if(A&&(A.success=="true")){m(null,"create success",function(){var C="/home.hdac";
